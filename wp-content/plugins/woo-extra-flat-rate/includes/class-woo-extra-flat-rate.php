...
class Woo_Extra_Flat_Rate {
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_action('wp_ajax_my_dismiss_flatrate_notice', $plugin_admin, 'my_dismiss_flatrate_notice');

		$get_notice = get_option( 'woo-extra-flat-rate-notice-dismissed' ) ;
		if( empty($get_notice ) ) {
			$this->loader->add_action('admin_notices', $plugin_admin, 'admin_notices_extra_flat_rate');
		}
	...
	private function define_public_hooks() {
		...
		if (isset($get_md_woocommerce_shipping_method_format) && !empty($get_md_woocommerce_shipping_method_format)) {
			$this->loader->add_filter('woocommerce_locate_template',  $plugin_public, 'md_adon_plugin_template', 1, 3 );
		}
	}
