...
function kadence_init_define_values(){
  ...
  if(isset($virtue_premium['post_word_count'])) { $excerptlength = $virtue_premium['post_word_count'];} else { $excerptlength = '40'; } 
  define('POST_EXCERPT_LENGTH', $excerptlength);
}

add_action('init', 'kadence_init_define_values');

function kadence_seo_switch() {
  global $virtue_premium;
   if(isset($virtue_premium['seo_switch'])) {
      if($virtue_premium['seo_switch'] == 1 ) {
        $useseo = true;
      } else {
        $useseo = false;
      }
...
function kadence_display_sidebar() {
   if (class_exists('woocommerce'))  {
        ...
        array('kadence_sidebar_on_archive_page','kadence_sidebar_on_shop_page','kadence_sidebar_on_shop_cat_page','kadence_sidebar_on_blog_post','kadence_sidebar_on_blog_page','kadence_sidebar_on_product_post', 'kadence_sidebar_on_staff_post','is_404','kadence_sidebar_on_home_page','kadence_sidebar_on_myaccount_page','is_cart','is_checkout',array('is_singular', array('portfolio')), array('is_singular', array('attachment')), array('is_tax', array('portfolio-type')), array('is_tax', array('portfolio-tag'))
        ),
  ...
  $sidebar_config = new Kadence_Sidebar(
    array('kadence_sidebar_on_blog_post','kadence_sidebar_on_archive_page_no_woo','kadence_sidebar_on_staff_post','kadence_sidebar_on_blog_page','is_404','kadence_sidebar_on_home_page', array('is_singular', array('portfolio')), array('is_singular', array('attachment')), array('is_tax', array('portfolio-type'))
      ),
    array('page-fullwidth.php','page-feature.php','page-portfolio.php','page-landing.php','page-staff-grid.php','page-testimonial-grid.php','page-contact.php', 'page-portfolio-category.php' )
  );
}
...
function kadence_sidebar_on_shop_page() {
  if( is_shop() ) {
  global $virtue_premium; 
    if(isset($virtue_premium['shop_layout']) && $virtue_premium['shop_layout'] == 'sidebar') {
        return false;
      } else {
        return true;
    }
...
function kadence_sidebar_on_shop_cat_page() {
  ...
  global $virtue_premium; 
    if(isset($virtue_premium['shop_cat_layout']) && $virtue_premium['shop_cat_layout'] == 'sidebar') {
        return false;
      } else {
        return true;
    }
  ...
  if( is_product()) {
    ...
    $postsidebar = get_post_meta( $post->ID, '_kad_post_sidebar', true );
      if(isset($postsidebar) && $postsidebar == 'yes') {
        ...
        global $virtue_premium; 
        if(isset($virtue_premium['product_sidebar_default']) && $virtue_premium['product_sidebar_default'] == 'yes') {
            return false;
          } else  {
            return true;
          }
...
function layout_class_names($classes) {
 global $virtue_premium;
  ...
  if(isset($virtue_premium['boxed_layout'])) {
    $layoutstyle = $virtue_premium['boxed_layout'];
  } else {
    $layoutstyle = 'wide';
  }
...
add_filter('body_class','layout_sticky_class');
function layout_sticky_class($classes) {
 global $virtue_premium;
  // add 'class-name' to the $classes array
...
add_filter('body_class','kt_sub_indicator');
function kt_sub_indicator($classes) {
 global $virtue_premium;
  if(isset($virtue_premium['show_subindicator']) && $virtue_premium['show_subindicator'] == 1) {
    $indicatorclass = 'kt-showsub-indicator';
  } else {
    $indicatorclass = '';
  }
