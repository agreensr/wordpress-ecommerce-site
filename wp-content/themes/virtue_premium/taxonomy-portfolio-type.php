		...
		if(isset($virtue_premium['portfolio_type_under_title']) && $virtue_premium['portfolio_type_under_title'] == '0') {
		    	...
		    	if(isset($virtue_premium['portfolio_tax_masonry']) && $virtue_premium['portfolio_tax_masonry'] == 1) {
                 ...
                 if($portfolio_order == 'menu_order' || $portfolio_order == 'title') {$p_order = 'ASC';} else {$p_order = 'DESC';}
		            ?> 
		<div id="portfoliowrapper" class="rowtight init-isotope" data-fade-in="<?php echo esc_attr($animate);?>" data-iso-selector=".p-item" data-iso-style="masonry" data-iso-filter="false"> 
		<?php global $wp_query;
				// get the query object
					$cat_obj = $wp_query->get_queried_object();
		 $termslug = $cat_obj->slug;
		query_posts(array( 'paged' => $paged, 'posts_per_page' => $portfolio_items, 'orderby' => $portfolio_order, 'order' => $p_order, 'post_type' => 'portfolio', 'portfolio-type' => $termslug) );
